...
- **Never call a UI or kernel primitive from an engine/frame hook — and do not
  ...
  not from the engine frame path. The same hook also fires far too rarely to
  drive a countdown (route A: 0 entries across a bare boot, a `--load-project`
  run and a 4000-frame `--sequencer` run), so anything timed from it never
  expires. **If you need "has N seconds passed", read the OS's own state**
  (`0x460d1e70` handle / `0x460d1e6c` countdown for a toast) instead of
  counting ticks yourself. NOTES "Session 93".
...
- **Scratch RAM at `0x80006a40+` is NOT reliable under live audio, and our
  ...
  reported it on screen), while it persisted perfectly in route A — which does
  not run the DSP/audio path for real. The block sits in the **DSP shared-RAM
...
- **When hardware and the emulator disagree, build a diagnostic, don't reason.**
  ...
  settled it on the first try. The emulator's blind spots (no UI tick, no real
  DSP path) are exactly where the bugs lived, so "emulator ALL GOOD" is evidence
  about the logic, never about the machine. Say which one you mean.
