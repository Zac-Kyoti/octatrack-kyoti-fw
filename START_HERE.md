...
| **DIRECT JUMP** pattern-change mode | **`DIRECTJUMP_V3` flashed 2026-09-14/15 — DID NOTHING** (combo dead, Session 60: stock `[PTN]`-held UI overlay NULLs the `[YES]` dispatch slot). **v4 flashed 2026-09-15 — combo reachable but "sort of works"** (Session 61: toast didn't close on `[PTN]` release + `dj_c` clobbered D7, breaking playhead-preservation; both fixed). **That v4 then threw a real hardware EXCEPTION (VEC:04, ADDR 0x400BF0F2) after a few `[PTN]` presses — TWICE**, on Session 61's toast-close hook and on Session 62's follow-up fix, proving Session 62's diagnosis wrong. **Actual root cause found (Session 64)**: a plain stack-discipline bug in the hook — it replayed a displaced `pea 0x400bf0f2` (whose pushed value must survive as an argument for the code right after it) and then did `rts`, which popped that pea'd value itself as a return address and jumped straight to it (our own read-only data, decoding as an illegal opcode) — on every single `[PTN]` release. Fixed by switching the detour to `jmp`-kind and ending with `pea ; jmp <addr>` instead of `rts`, matching this patch's own `djt_stock` idiom; confirmed by hand-reconstructing the old buggy pattern and showing the new emulator regression check fails against it exactly as expected. Every fix dynamically verified against the *real* stock layer-push/dispatch/release code (`tools/emu_directjump_v4.py`), not hand-built stubs. **Not yet reflashed** — a plain power cycle recovers a unit that hit the exception. ⚠️ RELOAD2's `rl_yes` shares the same dead `0x4005e4c8` hook — likely equally broken, not yet fixed. | `NOTES.md` "Session 15" + "Session 21" (+ continued) + **"Session 60"** + **"Session 61"** + **"Session 62"** + **"Session 64"** |
