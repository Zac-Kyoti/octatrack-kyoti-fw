...
| **DIRECT JUMP** pattern-change mode | **Sessions 60-64** (full trail in NOTES.md): `V3` flashed and did nothing → **v4** fixed reachability, then threw a hardware `EXCEPTION VEC:04 ADDR 0x400BF0F2` after a few `[PTN]` presses **twice** before the real cause (a stack-discipline bug) was found and fixed in Session 64 — **confirmed crash-free on hardware in Session 66**. Combo toggling and the fast (~1-step) switch timing are both **HW-confirmed working**. **⚠️ STILL OPEN (Sessions 66-67): with DIRECT JUMP ON, every manual pattern change starts the new pattern at step 1**, not the playhead position — two fix attempts on `dj_c`'s D7 write have both failed on hardware despite passing isolated emulator checks each time. **Session 67 is a HANDOFF, no code changed**: found a previously-unexamined per-track gate (`0x80006500[track]`, 17 refs, untraced) the trig-fire path depends on, got an inconclusive result trying to test the real per-tick trig-fire dispatcher (`0x400a3ca6`) in isolation, and concluded isolated single-function Unicorn testing has hit its limit for this bug — next step is `tools/emu_rtos.py` (the full real-scheduler, real-sequencer emulator this project already has) against a real project, comparing actual trig-fire timing per track across a real manual switch. Full detail + the exact next steps: `NOTES.md` "Session 67". ⚠️ RELOAD2's `rl_yes` shares the dead-`0x4005e4c8`-while-`[PTN]`-held mechanism v1-v3 had — likely equally broken, not yet fixed. | `NOTES.md` "Session 15" + "Session 21" (+ continued) + **"Session 60"** → **"Session 67" (HANDOFF)** |
