...
| **DIRECT JUMP** pattern-change mode | **`DIRECTJUMP_V3` flashed 2026-09-14/15 — DID NOTHING** (combo dead, Session 60 root cause: stock `[PTN]`-held UI overlay NULLs the `[YES]` dispatch slot). **v4 flashed 2026-09-15 — combo reachable, but "sort of works": toast didn't close on [PTN] release + pattern jumps didn't respect the playhead** (Session 61: `dj_c`, shared by every DJ build since Session 15, was clobbering D7 — a tick count two per-track phase loops consume right after the hook — with the raw step index; fixed by leaving D7 untouched). Both fixes dynamically verified against the *real* stock layer-push/dispatch/release code (`tools/emu_directjump_v4.py`), not just hand-built stubs. **Not yet reflashed.** ⚠️ RELOAD2's `rl_yes` shares the same dead `0x4005e4c8` hook — likely equally broken, not yet fixed. | `NOTES.md` "Session 15" + "Session 21" (+ continued) + **"Session 60"** + **"Session 61"** |
