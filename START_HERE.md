...
| **DIRECT JUMP** pattern-change mode | **Sessions 60-64** (full trail in NOTES.md): `V3` flashed and did nothing (stock `[PTN]`-held overlay NULLs `[YES]`'s dispatch slot) → **v4** fixed reachability, but then threw a hardware `EXCEPTION VEC:04 ADDR 0x400BF0F2` after a few `[PTN]` presses **twice**, on two different attempted fixes to the toast-close hook, before the real cause (a stack-discipline bug — a stray `rts` consuming a displaced `pea`'s pushed value as a return address) was found and fixed in Session 64 — **confirmed crash-free on hardware in Session 66**. **⚠️ Then (Session 66): with DIRECT JUMP ON, every manual pattern change started the new pattern at step 1**, not the playhead position — traced the per-track trigger-firing machinery (a free-running phase counter `0x80006604[track]`, read by a wholly separate function from the one Session 61 stopped at) to find it was always being seeded "fresh start" regardless of the (correctly-set) master step; `dj_c` now re-derives D7 as `resumeStep * DAT_80006628` — stock's own formula, with the resume step standing in for the full pattern length — fixing it. Every fix dynamically verified against the *real* stock code (`tools/emu_directjump_v4.py`, `tools/emu_directjump.py`), not hand-built stubs. **Not yet reflashed.** ⚠️ RELOAD2's `rl_yes` shares the dead-`0x4005e4c8`-while-`[PTN]`-held mechanism v1-v3 had — likely equally broken, not yet fixed. | `NOTES.md` "Session 15" + "Session 21" (+ continued) + **"Session 60"** → **"Session 66"** |
