...
"""
Recover the DSP56300 module load map from OUR OWN MAIN OS image.

Ported near-verbatim from refs/octa-bt-pt/tools/dsp_modmap.py (see that file's
own header for the full explanation of the boot sequence and record format --
kept here only where our own addresses/behaviour differ). Session 77: this is
step 2 of the cross-core SIDECHAIN handoff (NOTES.md "Session 77") -- confirm
the payload byte ranges on OUR image before pointing any dual-core emulator
work at them.

    python3 tools/dsp_modmap.py
    python3 tools/dsp_modmap.py --extract A 0x1aa4 out/comp_a.bin
    python3 tools/dsp_modmap.py --dumpmem A out/payload_a.mem
"""
import pathlib, sys
...
# Same addresses octa-bt-pt used -- our own dsp56300.md kb entry (boot table,
# fetched from octabam/octa-bt-pt against a shared 1.40C-family image) already
# cites these ColdFire source addresses/lengths; MKI/MKII share this image.
