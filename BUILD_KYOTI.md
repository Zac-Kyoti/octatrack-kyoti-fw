...
> emptied trigless lock** (`build_triglock.py` — built, never flashed). See
...
| `python3 tools/build_triglock.py` | `1.40C` (unchanged) | **Auto-remove an emptied trigless lock** — a LIVE-REC `[NO]`+knob erase that clears a step's last p-lock now also drops the now-purposeless trigless lock, instead of leaving it lit on the trig row indefinitely. On otherwise-stock 1.40C. *Active WIP — built and emulator-clean, never flashed* |
...
| **Auto-remove an emptied trigless lock** (`build_triglock.py`) | **emulator only** — built and clean 2026-09-20, never flashed |
