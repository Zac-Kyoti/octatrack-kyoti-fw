...
> — flashed but does not fix the originally reported bug), and **auto-remove an
> emptied trigless lock** (`build_triglock.py` — core behaviour hardware-confirmed;
> the current build refines it and awaits a hardware pass). See
...
| `python3 tools/build_triglock.py` | `1.40C` (unchanged) | **Auto-remove an emptied trigless lock** — a LIVE-REC `[NO]`+knob erase that clears a step's last p-lock now also drops the now-purposeless trigless lock, instead of leaving it lit on the trig row indefinitely. An empty trigless lock placed deliberately with `FUNC`+`TRIG` is left alone. On otherwise-stock 1.40C. *Core behaviour hardware-confirmed 2026-09-21; the placeholder-preserving refinement is emulator-clean and awaits a hardware pass* |
...
| **Auto-remove an emptied trigless lock** (`build_triglock.py`) | **partly hardware-confirmed.** The handler was found by tracing the gesture on the unit after three builds aimed at the wrong code did nothing; flashed 2026-09-21 and **all four gesture tests passed**. The current build then moved the detour onto the erase store so a deliberate `FUNC`+`TRIG` empty placeholder is not deleted by an unrelated `[NO]`+knob — emulator-validated against real exports (8/8, including a real placeholder), **not yet flashed** |
