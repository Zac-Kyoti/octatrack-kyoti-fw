...
| `FUN_4005a0e0(text)` | **bare text popup**, h=18px, own handle `0x460d1e64`, font `0x400ba862`, ctor `FUN_4005829c(w, 0x12, 0,0, 0xa, FUN_40056bc0)`, close cb `FUN_40056bc0`, **no timeout**. DEAD CODE in stock (0 callers). **DIRECT JUMP v2** revives it + `dj_tick2` (a frame countdown spliced at `0x400522ca` inside the per-frame fn `@0x40052200`) for a box-free auto-dismiss toast. 🛑 **DO NOT CALL ANYTHING UI- OR KERNEL-FACING FROM `0x400522ca` / `FUN_40052200`** (C, ours, Session 93 — **hardware-proven by a crash**). QLREC's `qlr_tick` called `FUN_4005a2b8`/`FUN_40056bec` from this site; both bottom out in **`FUN_40000c3c`, the kernel post/wake** (masks to `0x2700`, ring-writes, then marks a blocked task runnable at `+0x4c` and pokes the ready-list head `0x800068d8`). From the engine frame context that froze the panel and left the audio path unserviced — a loud persistent HF crackle — after a few uses. Current DIRECT JUMP (v4/v5) does **not** splice it; only the dead v2 does. ⚠️ **Retract the Session 21 description of this as a "per-control-frame handler" that "ticks every frame, playing or stopped": its rate is unmeasured and evidently low** (route A: 0 entries across a bare boot, a `--load-project` run and a 4000-frame `--sequencer` run), which is separately why a countdown driven from here never expired. Its one call site `0x40061e8e` has no static branch or table reference — it is entered only through a computed switch. What the site *is*: the stock **SOFT MUTE release watchdog** tick — `FUN_40008f84` writes 45 to `relparam_46c7dfba[t]` (8 longs, `0x46c7dfba..dfda`) and `0x400522de..e6` decrements it, force-freeing a voice still in release state 2. |
| `FUN_4005a2b8(text, dur)` | the OS notification (= ems-octakit `GK_STOCK_NOTIFICATION_SHOW`, stock's "PART n RELOADED"). `dur > 0` → self-timing, no handle to manage (**DIRECT JUMP v3**, `patch_reload2` `rl_yes`). **`dur ≤ 0` → persistent** (the `0x4005a334` branch, no countdown); handle `0x460d1e70`, close explicitly with **`0x40056bec()`** (no args, no-op if none open). Window auto-sizes to `textpx + 15`, h=18px. ⚠️ `dur ≤ 0` tail-jumps into `FUN_40031494`, the **modal window/overlay stack** — it HUNG a real MKI (NOTES Session 50); never pass it. **QUANTIZE LIVE REC** now re-arms a short `dur > 0` instead (NOTES Session 50/51) and, since Session 92, closes explicitly when its own `LIVE_TICKS` window expires. **`dur`'s clock — MEASURED: one unit = 1/60 s (C, ours, Session 93).** `dur` is stored at `0x460d1e6c` and counted down by `0x40056c28` (at zero it `bra`s into `FUN_40056bec`, so the OS closes the toast itself), called from **`0x40056c40` — a TASK entry point**, created at `0x4004040e` (prio 3, stack `0x460d51d4`+`0x800`, TCB `0x460d59d4`), on **message type 1** (the byte at `0x400a727a`) from queue `0x460d1664`. That message is posted by the **DTIM1 ISR `0x40055cb8`** (ends in `rte`; clears DTER `0xfc074003`) through a ÷2 prescaler at `0x400c0cf0` (reloaded with 2 at `0x40055cf4`). DTIM1 is programmed once at `0x40040498`: `DTRR 0xfc074004 = 68750`, `DTMR 0xfc074000 = 0x001d` (RST=1, CLK=2 = bus clock ÷16, FRR=1, ORRI=1, PS=0) → `(68750+1)×16 / f_bus`. At `f_bus = CPU/2 = 132 MHz`: DTIM1 = 8.3335 ms (119.998 Hz), **UI tick = 16.667 ms = 59.999 Hz**. So **`dur` × 1/60 s**: `0x30` = **0.800 s**, `0x44` = 1.13 s, `0x60` = 1.60 s. Cross-check: stock's own values are **87×`0x30`**, 7×`0x60`, 3×`0x44`, 2×`0x5a` — `0x30` being the house length at 0.8 s is consistent; the alternative `f_bus = 264 MHz` would make it 0.4 s, implausibly brief for "PART n RELOADED". **`0x460d1e70` (handle) + `0x460d1e6c` (countdown) are the OS's own "is a toast on screen" pair** — readable from a key handler and maintained in the UI task; QLREC gates its flip on them rather than ticking anything itself. |
